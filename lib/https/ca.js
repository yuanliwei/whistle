...
async function createSelfCertAsync(hostname) {
  var serialNumber =
    crypto
      .createHash('sha1')
      .update(hostname + RANDOM_SERIAL, 'binary')
  ...
  let subject = await new Promise((resolve) => {
    ...
    let socket = tls.connect({
      ...
      socket.destroy()
      let subject = Object.keys(cert.subject).reduce((p, v) => (p.push({
        shortName: v,
        value: cert.subject[v],
...
exports.createCertificate = async function (hostname) {
  hostname = getDomain(hostname);
  var cert = cachePairs.get(hostname); // 确保使用自己生成的证书，防止把用户证书下载出去
  if (!cert) {
    cert = await createSelfCertAsync(hostname);
...
async function getOrCreateCertAsync(servername) {
  var requestCert = servername[0] === ':'
  if (requestCert) {
    servername = servername.substring(1)
  }
  ...
  if (!cert) {
    ...
    cert = getCacheCert(servername)
    if (!cert) {
      cert = await createSelfCertAsync(servername)
      cachePairs.set(servername, cert)
    }
  }
  return requestCert
...
var SNICallback = async function (servername, cb) {
  var options = await getOrCreateCertAsync(servername);
  if (!options._ctx) {
    try {
